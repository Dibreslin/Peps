...
def do_login(email, password):
    """Función de login (temporal - luego se conecta a Supabase)"""
    if email and password:
        # Login hardcodeado para probar
        if email == "marina@peps.com" and password == "123456":
            st.session_state.authenticated = True
            st.session_state.user = email
            st.session_state.user_name = "Marina Breslin"
            return True
        elif email == "admin@peps.com" and password == "admin123":
            st.session_state.authenticated = True
            st.session_state.user = email
    ...
    with col2:
        with st.container(border=True):
            ...
            with st.form("login_form", clear_on_submit=False):
                ...
                with col_btn2:
                    st.button("❓ Olvidé mi contraseña", use_container_width=True, disabled=True)
            ...
            st.divider()
            st.caption("📝 Datos de prueba: marina@peps.com / 123456")
            st.caption("👤 admin@peps.com / admin123")
...
menu = st.sidebar.radio(
    "📋 Navegación",
    ["📅 Dashboard", "👤 Pacientes", "📆 Turnos", "💰 Pagos", "📊 Reportes"],
    index=0
)
...
if menu == "📅 Dashboard":
    ...
    with col1:
        ...
        ingresos = [450000, 520000, 610000, 580000, 720000, 680000]
    ...
    with col2:
        st.markdown("#### 📊 Resumen")
        st.metric("Total Ingresos", "$ 3.560.000", delta="+12%")
        st.metric("Promedio por Sesión", "$ 30.000", delta="+5%")
...
# ============================================
    ...
    st.dataframe(turnos.style.applymap(color_estado, subset=["Estado"]), use_container_width=True, hide_index=True)
...
# ============================================
    ...
    st.warning("🔄 Esta sección está en desarrollo. Próximamente: reportes de ingresos, asistencia y más.")
    
    col1, col2 = st.columns(2)
    with col1:
        st.metric("📈 Total Pacientes", "24", delta="+4 este mes")
    with col2:
        st.metric("📆 Promedio de Sesiones", "4.2 por paciente", delta="+0.8")
